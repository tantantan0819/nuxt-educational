    ...
    <div class="add">
      <el-dialog title="" :visible.sync="outerVisible" class="school_dialog add">
        ...
        <div class="add_content">
          <el-form :model="cvForm2" status-icon :rules="rules" ref="cvForm2" label-width="300px"
                   label-position="top">
            <el-form-item label="入学时间" prop="admission_date">
              <el-date-picker type="date" format="yyyy-MM-dd" value-format="yyyy-MM-dd" placeholder="请输入您的入学时间"
                              v-model="cvForm2.admission_date"
                              class="widthAll"></el-date-picker>
            </el-form-item>
            <el-form-item label="毕业时间" prop="graduaction_date">
              <el-date-picker type="date" format="yyyy-MM-dd" value-format="yyyy-MM-dd" placeholder="请输入您的毕业时间"
                              v-model="cvForm2.graduaction_date"
                              class="widthAll"></el-date-picker>
            </el-form-item>
            <el-form-item label="毕业/就读院校" prop="school_name">
              <el-input v-model="cvForm2.school_name" placeholder="请选择您的毕业/就读院校" autocomplete="off" readonly
                        @focus="innerVisible=true"></el-input>
            </el-form-item>
            <el-form-item label="毕业/就读专业" prop="major_id">
              <el-select v-model="cvForm2.major_id" class="widthAll" placeholder="请选择您的毕业/就读专业">
                <el-option v-for="(item,index) in select.major" :key=index :label="item.major_name_cn"
                           :value="item.id"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="学历" prop="qualification">
              <el-select v-model="cvForm2.qualification" class="widthAll" placeholder="请选择您的学历">
                <el-option v-for="(item,index) in select.record" :key=index :label="item.cvalue_cn"
                           :value="item.id"></el-option>
              </el-select>
            </el-form-item>
          ...
          <div class="footer_button">
            <span @click="cancel('cvForm2')"><i>取消</i></span>
        ...
        <el-dialog width="650px" title="" :visible.sync="innerVisible" append-to-body>
          ...
          <div class="add_content school_content">
            <el-input class="input-with-select" placeholder="请输入搜索的学校名称" v-model="search" @keyup.enter.native="searchSchool">
              ...
              <el-table :data="schoolTable" stripe style="width: 100%">
                <el-table-column prop="" label="选择" width="70">
                  <template slot-scope="scope">
                    <el-radio v-model="selectSchool" :label="schoolTable[scope.$index].id" @change="handleEdit(scope.$index)" class="schoolRadio"></el-radio>
                  </template>
                </el-table-column>
    ...
    export default {
        ...
        data() {
            return {
                ...
                rules: {
                    admission_date: [
                        {required: true, message: '请输入您的入学时间', trigger: 'blur'}
                    ],
                    graduaction_date: [
                        {required: true, message: '请输入您的毕业时间', trigger: 'blur'}
                    ],
                    school_id: [
                        {required: true, message: '请输入您的毕业/就读院校', trigger: 'blur'}
                    ],
                    school_name: [
                        {required: true, message: '请输入您的毕业/就读院校', trigger: 'blur'}
                    ],
                    major_id: [
                        {required: true, message: '请选择您的毕业/就读专业', trigger: 'blur'}
                    ],
                    qualification: [
                        {required: true, message: '请选择您的学历', trigger: 'blur'}
                    ],
                    average_result: [
                        {required: true, message: '请输入您的平均成绩', trigger: 'blur'}
                    ]
                }
            }
        },
