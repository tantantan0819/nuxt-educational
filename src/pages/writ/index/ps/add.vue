        ...
        <div class="cv_step2">
            ...
            <div class="cv_content ps_add ps_add2">
                ...
                <div class="add_box">
                    <div
                        class="add_item"
                        v-for="(item,index) in plan.apply"
                        :key="index"
                        :class="{'active': index == addIndex}"
                    >
                        <div class="add_title">
                            申请方案{{index+1}}
                            <span v-if="isView" @click="viewDetail(index,item.id)">查看详情</span>
                        </div>
                            ...
                            <tr>
                                ...
                                <td colspan="3">
                                    <a
                                        class="reviewBtn"
                                        target="_blank"
                                        :href="view_href+detail.cas_file"
                                        v-if="detail.cas_file"
                                    >查看CAS</a>
                                </td>
                            </tr>
                            <tr>
    ...
    methods: {
        ...
        next() {
            if (this.isNext) {
                ...
                this.$message({
                    message:
                        "该合同下暂无可申请的PS方案哦~ 如有疑问，请联系你的顾问老师！",
                    type: "warning"
                });
            }
        },
        ...
        viewDetail(index, id) {
            //获取院校实时更新
            http.get("/customer-apply-update/list", { apply_id: id }).then(
                res => {
                    this.update = res;
                }
            );
